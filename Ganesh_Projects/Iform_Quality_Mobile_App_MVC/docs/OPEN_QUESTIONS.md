...
- **Demo data**: demo tenant has 2 projects (PRJ-1002, PRJ-1003), 3 IPOs (IPO-3001/3002/4001), 7 queries (SQ-0001..0007 with backdated `RaisedDate` for severity variety), 2 EOTs (EOT-01 Approved, EOT-02 Draft), and users in all BRD roles: `admin` (TenantAdmin), `manager` (Manager), `engineer1`/`engineer2` (SiteEngineer). Repeatable via `scripts/seed-demo-data.ps1` (idempotent; optional `-SqlConnectionString` backdates raised dates for the delay report). Demo logins: `admin/Admin@12345`, `manager/Mgr@12345`, `engineer1/Eng@12345`, `engineer2/Eng2@12345`, `superadmin/Admin@12345`.
- **Roles vs BRD**: BRD specifies two user roles — Site Engineer and Manager. The app keeps internal role values (`SiteEngineer`, `Manager`, `TenantAdmin`, `SuperAdmin`) but displays BRD-friendly names in User Management (`Site Engineer`, `Manager`, `Tenant Admin`) via `AppRoles.DisplayName`. The tenant admin plus the SaaS super-admin are extensions beyond the BRD's two roles.
- **Login lockout**: `PasswordSignInAsync(lockoutOnFailure: true)` locks an account for a few minutes after 5 failed attempts (message: "Account is locked out"). If a demo login reports an error, first confirm the app is running (`http://localhost:5246/health` → 200) and that the account isn't locked out.
